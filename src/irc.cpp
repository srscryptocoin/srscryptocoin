...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #SrsCoinTEST2\r");
            Send(hSocket, "WHO #SrsCoinTEST2\r");
        } else {
            // randomly join #SrsCoin00-#SrsCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #SrsCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #SrsCoin%02d\r", channel_number).c_str());
        }
