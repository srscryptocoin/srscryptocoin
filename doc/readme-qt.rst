SrsCoin-qt: Qt4 GUI for SrsCoin
===============================
...
Alternatively, install Qt Creator and open the `SrsCoin-qt.pro` file.

An executable named `SrsCoin-qt` will be built.
...
A warning for people using the *static binary* version of SrsCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of SrsCoin is linked against libdb5.3.
...
installed causes SrsCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
